...
/**
 * ⚠️ SỬA (review hiệu năng): `maxPoolSize` trước đây hard-code = 10 — khá
 * thấp cho 1 app có nhiều user/department cùng thao tác đồng thời (mỗi
 * connection trong pool chỉ xử lý được 1 query tại 1 thời điểm, request
 * thứ 11 trở đi phải xếp hàng chờ connection rảnh dù MongoDB hoàn toàn có
 * thể phục vụ song song).
 *
 * Nay đưa ra `MONGO_MAX_POOL_SIZE`/`MONGO_MIN_POOL_SIZE` qua env để có thể
 * tinh chỉnh theo tải thực tế đo được (benchmark) mà không cần sửa code,
 * default nâng lên 20 (điểm khởi đầu hợp lý cho app quy mô vừa, vẫn nên
 * benchmark lại khi traffic tăng).
 * `minPoolSize` giữ sẵn vài connection ấm (không phải mở mới từ 0 mỗi lần
 * traffic tăng đột ngột), mặc định 2.
 */
const mongoOptions: mongoose.ConnectOptions = {
  autoIndex: true,              // bật index dev, prod có thể tắt
