...
/**
 * ⚠️ ĐÃ BỎ field `dbTime`/`serviceTime`/`controllerTime`: middleware trước
 * đây khai báo sẵn 3 field đo thời gian theo từng layer, nhưng không có
 * service/controller nào thực sự cập nhật giá trị → luôn ghi 0, không phản
 * ánh gì cả. Xoá khỏi schema để tránh dữ liệu "rỗng nhưng trông như đã đo".
 * Muốn đo lại theo layer: phải chủ động wrap thời gian ở service/controller
 * rồi mới thêm lại field tương ứng.
 */
const ApiPerformanceSchema = new Schema(
  {
    method: String,
