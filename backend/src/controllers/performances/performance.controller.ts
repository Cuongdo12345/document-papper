 ...
 */
/**
 * ⚠️ LƯU Ý VỀ SỐ LIỆU (từ khi thêm batching + sampling ở
 * `performanceLogBuffer.ts`):
 *  - Request CHẬM (`isSlow`) và request LỖI (status >= 400) luôn được ghi
 *    đầy đủ 100%, nên `slowCount`/`errorCount` vẫn CHÍNH XÁC tuyệt đối.
 *  - Request bình thường chỉ được ghi theo tỉ lệ sample (mặc định 10%, xem
 *    `PERF_SAMPLE_RATE`), nên `totalCalls`/`avgTime` là SỐ ƯỚC LƯỢNG, không
 *    phải đếm chính xác 100% lưu lượng thật. Nếu cần tổng lượt gọi chính
 *    xác tuyệt đối, phải lấy từ nguồn khác (vd access log của reverse
 *    proxy/API gateway), không nên dựa vào collection này.
 */
export const getPerformanceDashboard = async (req: Request, res: Response) => {
  const { from, to } = req.query;
