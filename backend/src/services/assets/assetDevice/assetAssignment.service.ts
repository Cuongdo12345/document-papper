...
/**
 * DEV-022/RV06-08: cả 3 hàm assign/transfer/return dùng chung pattern đọc
 * (`Asset.findOne`) → sửa field trong memory → `asset.save({session})`.
 * Mongoose mặc định bật optimistic locking qua `versionKey` (`__v`) khi
 * `.save()` 1 document đã đọc trước đó — nếu 2 request cùng sửa 1 asset gần
 * như đồng thời (VD 2 nhân viên IT thao tác cùng lúc), request thua cuộc sẽ
 * nhận `VersionError`. Trước đây KHÔNG bắt riêng lỗi này — rơi xuống nhánh
 * 500 "lỗi không xác định" của `error.middleware.ts` (message thư viện khó
 * hiểu, sai status). Bắt riêng ở đây, trả `409 Conflict` rõ nghĩa — an toàn
 * dữ liệu vẫn được Mongoose versioning đảm bảo dù có bắt lỗi hay không (bắt
 * lỗi chỉ cải thiện UX của response, không đổi hành vi ghi dữ liệu).
 */
const runAssignmentTransaction = async <T>(
  ...
  try {
    ...
    if (err instanceof mongoose.Error.VersionError) {
      throw ApiError.conflict(
        "Tài sản vừa được người khác cập nhật, vui lòng tải lại thông tin mới nhất và thử lại",
      );
    }
    throw err;
  }
...
export const assignAssetService = async (
  ...
  // "ASSIGN".
  await runAssignmentTransaction(async (session) => {
