...
// // duy nhất là giữ thời gian xử lý gần bằng nhánh "user tồn tại nhưng sai mật
// // khẩu", giảm timing side-channel cho việc dò username hợp lệ qua độ trễ phản
// // hồi (điểm đã ghi nhận ở review — mục 6 "timing side-channel forgotPassword",
// // áp dụng tương tự cho login). Đây KHÔNG phải hash thật của tài khoản nào.
// const DUMMY_PASSWORD_HASH =
//   "$2b$10$CwTycUXWue0Thq9StjUM0uJ8n7wOwGmhchXvL0J6HqL0Yqz8G9c1O";
...
//    *
//    * ⚠️ SỬA (review Auth module):
//    *  - Đổi lỗi "quá nhiều yêu cầu" từ `ApiError.badRequest` (400) sang
//    *    `ApiError.tooManyRequests` (429) — đúng semantics HTTP, `ApiError` đã
//    *    có sẵn factory method này (`ApiError.ts`) nhưng chưa được dùng ở đây.
//    *  - Nhánh "user không tồn tại/bị khoá" giờ chạy thêm 1 query "vô hại"
//    *    (`PasswordResetToken.countDocuments` với `user` không tồn tại — luôn
//    *    trả 0, không ghi gì) để thời gian xử lý gần hơn với nhánh "user tồn
//    *    tại", giảm timing side-channel dùng để dò username hợp lệ.
//    */
