...
/**
 * 📌 GET ALL TEMPLATES (đọc-only) — FE-04 (Documents Core UI): trước đây
 * `createWorkflowTemplate` là hàm DUY NHẤT thao tác với `WorkflowTemplate`,
 * không có cách nào liệt kê lại các template đã tạo (không seed script,
 * không endpoint GET) — chặn thẳng màn hình "Submit vào workflow" (cần
 * chọn `templateId`, xem `SubmitWorkflowDTO`) vì không có dữ liệu để hiển
 * thị dropdown. Bổ sung THUẦN ĐỌC, không đổi hành vi `createWorkflowTemplate`
 * hay bất kỳ hàm nào khác trong file này. Chỉ trả template còn active —
 * template đã tắt (`isActive:false`) không nên hiển thị cho người dùng chọn
 * để submit workflow mới (không ảnh hưởng workflow đã submit trước đó bằng
 * template này, vì `WorkflowInstance` đã copy nguyên `steps[]` tại thời
 * điểm submit — xem `submitWorkflow` bên dưới).
 */
 ...
 *     `UNDER_MAINTENANCE`.
 *   - `CHECK_DAMAGE`   → tìm document `PROPOSE_REPAIR` gốc qua
 ...
 *
 * 🔧 DEV-005/IMP-005 (C-02=RV05-01=ARCH-12, xác nhận nghiệp vụ với chủ dự
 * án 2026-09-01, sau 1 lần xác nhận SAI đã revert): nhánh này trước đây bắt
 * theo `CONFIRM_STATUS`, nhưng theo `documentRules.ts` (business rule ĐÚNG,
 * KHÔNG đổi) `CONFIRM_STATUS` chỉ tham chiếu được `PROPOSE_INK` — không
 * liên quan gì tới `PROPOSE_REPAIR`/`Asset.status`. `CHECK_DAMAGE` mới là
 * REPORT đúng rule tham chiếu `PROPOSE_REPAIR` (`documentRules.ts:16-20`),
 * nên đây mới là subType thực sự đóng luồng sửa chữa. Đã đổi điều kiện
 * (KHÔNG đổi logic bên trong — cùng field `meta.repairResult`, cùng
 * `resolveAssetMaintenanceService`).
 *
 ...
 *
 * ⚠️ NÂNG CẤP (2026-09-10, DEV-038 đề xuất #2 — user yêu cầu làm tiếp): thêm
 * tham số `isAdmin` (optional, mặc định `false` — KHÔNG đổi hành vi cũ nếu
 * không truyền, mọi lời gọi hiện có/test cũ vẫn chạy đúng y hệt). Trước đây
 * NGAY CẢ ADMIN cũng bị chặn 403 nếu role của họ không khớp `step.role` —
 * nếu toàn bộ user mang role được chỉ định ở 1 bước đều bị vô hiệu hoá/nghỉ
 * việc, document kẹt VĨNH VIỄN không ai xử lý được. `isAdmin===true` cho
 * phép bỏ qua check role — nhưng vẫn PHẢI ghi log rõ ràng đây là "duyệt
 * thay" (không lặng lẽ giả vờ đúng role) bằng cách gắn tiền tố vào
 * `step.comment` — `step.approvedBy` vẫn lưu ĐÚNG user thật thực hiện, nên
 * audit trail qua `WorkflowStepsView`/lịch sử vẫn minh bạch tuyệt đối.
 */
export const approveStep = async (
  ...
  const isOverride = !!step.role && step.role !== role;
  ...
  step.comment = isOverride
    ? `[ADMIN duyệt thay role "${step.role}"]${comment ? ` ${comment}` : ""}`
    : comment;
 ...
 * `approveStep`.
 *
 * ⚠️ NÂNG CẤP (2026-09-10, DEV-038 đề xuất #2) — thêm tham số `isAdmin`,
 * cùng lý do/cách làm với `approveStep` (xem giải thích đầy đủ ở đó).
 */
export const rejectStep = async (
  ...
  step.approvedBy = userId;
  step.comment = isOverride
    ? `[ADMIN từ chối thay role "${step.role}"]${comment ? ` ${comment}` : ""}`
    : comment;
  step.approvedAt = new Date();
...
/**
 * 📌 GET WORKFLOW HISTORY — "Lịch sử duyệt" (2026-09-10, user yêu cầu trực
 * tiếp). KHÁC `getPendingApprovalsForRole` ở 2 điểm:
 *
 *   1. `getPendingApprovalsForRole` CHỈ khớp `status:"pending"` VÀ đúng
 *      `steps[currentStep].role` — nghĩa là "việc ĐANG chờ TÔI duyệt ngay
 *      bây giờ". Hàm này khớp BẤT KỲ phần tử nào trong `steps[]` có
 *      `role` trùng (`{"steps.role": actor.role}` — Mongo tự hiểu là "match
 *      nếu MỘT phần tử bất kỳ của mảng thoả field này", multikey index) —
 *      tức "mọi workflow mà role của tôi từng/đang phải xử lý", bất kể
 *      workflow đó hiện đang ở bước nào hay đã kết thúc chưa. Vì vậy 1
 *      workflow đã đi QUA bước của role này (approved/rejected bởi role đó)
 *      rồi chuyển sang bước khác VẪN xuất hiện ở lịch sử — đúng ngữ nghĩa
 *      "lịch sử", không phải "việc còn tồn đọng".
 *   2. Không hard-code `status:"pending"` — nhận thêm filter `status` TUỲ
 *      CHỌN (`QueryWorkflowHistoryDTO`) để xem riêng từng loại kết quả
 *      (đã duyệt/bị từ chối/đã huỷ/hoàn tất/vẫn đang chờ).
 *
 * `actor.isAdmin`: ADMIN xem TOÀN BỘ lịch sử, không lọc theo role — cùng
 * convention "ADMIN xem tất cả" đã áp dụng cho Audit Logs/Import History
 * (`excel.controller.ts` `getImportHistory`). Khác `getPendingApprovalsForRole`
 * (giữ NGUYÊN hành vi cũ — không sửa hàm đó, ADMIN gọi `/pending` vẫn trả
 * rỗng như trước giờ vì không step nào có `role:"ADMIN"`) — hàm MỚI này
 * không kế thừa giới hạn đó, vì mục đích của "lịch sử" đúng nghĩa là công
 * cụ giám sát/tra cứu, không phải hộp thư công việc cá nhân.
 *
 * Sort `createdAt: -1` (MỚI NHẤT trước) — ngược với `/pending` (`createdAt: 1`,
 * cũ nhất trước vì đó là hàng đợi công việc, ưu tiên xử lý việc chờ lâu
 * nhất trước).
 */
export const getWorkflowHistoryForUser = async (
  actor: { role: string; isAdmin: boolean },
  query: any = {},
) => {
  ...
  const filter: any = actor.isAdmin ? {} : { "steps.role": actor.role };
