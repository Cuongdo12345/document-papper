...
| 7/21 model không có index ngoài `_id` (`RefreshToken`, `Role`, `Permission`, `Policy`, `WorkflowTemplate`, `WorkflowInstance`, `Upload`) — nay 7/31, xem DEV-071 | Missing index | Phase 04 §10, Phase 10 PERF-04 |
...
| TD-21 | Database | 7/21 model thiếu index ngoài `_id` (nay 7/31, xem DEV-071) | Rủi ro hiệu năng tăng dần, mức độ khác nhau theo model | LOW–MEDIUM | Phase 04 §10, Phase 10 PERF-04 | Ưu tiên `WorkflowInstance`/`RefreshToken` trước (đã có TD-07/TD-11) | THẤP |
