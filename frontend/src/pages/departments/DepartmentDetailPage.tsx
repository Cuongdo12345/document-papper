...
import { useNavigate, useParams } from 'react-router-dom';
import { Card, Descriptions, Skeleton, Empty, Result, Typography } from 'antd';
import dayjs from 'dayjs';
...
// ✅ v2 — tái dùng nguyên component đã build cho List, KHÔNG viết form/schema riêng
import { DepartmentEditModal } from './components/DepartmentEditModal';

import { useDepartmentDetail, useDeleteDepartment } from '../../hooks/queries/useDepartment';
import { DEPARTMENT_DELETE_ROLES } from '../../constants/departments/department.permissions';

const { Text } = Typography;

// ─────────────────────────────────────────────────────────────────────────────
// DepartmentDetailPage — SCR-15 (v2 — Edit qua Modal, không còn Inline Edit)
...
export function DepartmentDetailPage() {
  ...
  // ── Local state — chỉ còn 1 boolean, không cần Form/schema/useEffect pre-fill ──
  ...
  return (
    <>
      <AppPageHeader
        ...
        // ✅ v2 — KHÔNG cần ẩn actions khi mở Modal (Modal là overlay, không xung đột)
        actions={[
          {
            key: 'edit',
            label: 'Chỉnh sửa',
      ...
      {isLoading ? (
        ...
        // ── Information Sections — luôn Read-only, Edit nằm trong Modal riêng ──
        <Card>
          <Descriptions column={1} labelStyle={{ width: 160 }}>
            <Descriptions.Item label="Mã phòng ban">
              <Text strong>{department!.code}</Text>
      ...
      {/*
        ✅ v2 — DepartmentEditModal tái dùng nguyên component từ List.
        Gate qua isEditModalOpen vì Modal tự suy open = (department !== null);
        department ở Detail luôn có giá trị sau khi load, nên phải tự gate lại
        bằng state phụ trước khi truyền vào prop `department` của Modal.
      */}
      <DepartmentEditModal
        department={isEditModalOpen ? (department ?? null) : null}
